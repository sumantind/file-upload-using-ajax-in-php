    ...
    <?php
      ...
      if(!empty($_POST["send"])) {
        ...
        // if(mail($toMail, $subject, $message, $header)) {
            ...
            $sql = $connection->query("INSERT INTO contacts_list(name, email, phone, subject, message, password, sent_date)
            VALUES ('{$name}', '{$email}', '{$phone}', '{$subject}', '{$message}', '{$password}', now())");
            if(!$sql) {
